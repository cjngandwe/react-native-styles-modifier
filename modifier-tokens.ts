...
export class ModifierWithTokens<
...
> extends Modifier<T> {
  ...
  shadowToken(key: keyof DesignTokens["shadows"]): this {
    ...
    // Apply shadow properties directly to styles
...
export function createModifierWithTokens<T extends StyleSheetLike>(
  root: T,
  tokens?: DesignTokens,
): ModifierWithTokens<T> {
  return new ModifierWithTokens(root, tokens);
}
